...
export async function socketHandler(fastify: FastifyInstance)
{
	...
	io.on("connection", async (socket) => {
		...
		const token = cookies["token"]; // si ton cookie s'appelle "jwt"
		...
		try {
			...
			// je recrée le payload depuis le token pour connaitre l'id
			...
			db.prepare('UPDATE users SET isConnected = ? WHERE id = ?').run(1, userId);
		...
		socket.on("disconnect", async () => {
				...
				db.prepare('UPDATE users SET isConnected = ? WHERE id = ?').run(0, socket.data.userId);
			});
