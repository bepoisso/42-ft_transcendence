...
openssl genrsa -out ./certs/filebeat-key.pem 4096

# Verify filebeat private key was created
if [ ! -f ./certs/filebeat-key.pem ]; then
    echo "Error: filebeat private key generation failed"
    exit 1
fi

# Create config file for filebeat certificate
cat > ./certs/filebeat.conf << EOF
[req]
distinguished_name = req_distinguished_name
req_extensions = v3_req
prompt = no
...
# Generate filebeat certificate signing request
openssl req -new -key ./certs/filebeat-key.pem -out ./certs/filebeat.csr -config ./certs/filebeat.conf

# Verify CSR was created and CA files exist
if [ ! -f ./certs/filebeat.csr ]; then
    echo "Error: filebeat CSR generation failed"
    exit 1
fi
if [ ! -f ./certs/ca-cert.pem ] || [ ! -f ./certs/ca-key.pem ]; then
    echo "Error: CA files not found"
...
# Generate filebeat certificate signed by CA
openssl x509 -req -in ./certs/filebeat.csr -CA ./certs/ca-cert.pem -CAkey ./certs/ca-key.pem -CAcreateserial -out ./certs/filebeat-cert.pem -days 365 -extensions v3_req -extfile ./certs/filebeat.conf
